...
function interfaceSubtitle(item: WiredInterface) {
  if (item.connected) {
    return item.ip ? `Connected [${item.ip}]` : 'Connected'
  }
    ...
    <v-card-text class="pa-3">
      <v-container fluid class="pa-0">
        <v-row no-gutters align="center">
          ...
          <v-col>
            <div class="text-subtitle-1 font-weight-medium">Wired</div>
            <div class="text-body-2 text-medium-emphasis">
              {{ wired.interfaces.length === 0 ? 'No wired interfaces found' : `${wired.interfaces.length} interface${wired.interfaces.length === 1 ? '' : 's'}` }}
            </div>
          </v-col>
        </v-row>
