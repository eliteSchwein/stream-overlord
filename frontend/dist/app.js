...
%o`, message, error2, detail);
  ...
  function addEasing(name2, easing) {
    if (easingFunctions.get(name2)) {
      return;
    }
    easingFunctions.set(name2, easing);
  }
  function getEasing(name2) {
    return easingFunctions.get(name2) ?? ((value) => value);
  }
  function getRandom() {
    const min = 0, max = 1;
  ...
  function drawLinkLine(params) {
    ...
    if (shadow2.enable) {
      const shadowColor = rangeColorToRgb(shadow2.color);
      if (shadowColor) {
        context.shadowBlur = shadow2.blur;
        context.shadowColor = getStyleFromRgb(shadowColor);
      }
  ...
  __publicField(AlertController, "targets", ["icon", "content", "sound", "video", "contentContainer", "logo", "iframe"]);
