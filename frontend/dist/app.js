(() => {
  ...
  var WebsocketClient = class {
    constructor() {
      ...
      __publicField(this, "hasReloadedAfterReconnect", false);
      __publicField(this, "baseDelayMs", 1e3);
      __publicField(this, "maxDelayMs", 3e4);
      // soft reload guard
      __publicField(this, "softReloadInProgress", false);
    }
    async connect() {
      const config2 = getConfig(/websocket/g)[0];
      this.url = `ws://${window.location.hostname}:${config2.port}`;
      this.openWebsocket();
      await sleep(250);
    }
    openWebsocket() {
    ...
    async tryReconnect() {
      ...
      while (this.reconnecting) {
        ...
        try {
          delete window.websocket;
          this.openWebsocket();
          const opened = await this.waitForOpen(5e3);
          if (opened) {
            return;
          }
          try {
    ...
    async softReloadPage() {
      ...
      try {
        ...
        const nextScripts = Array.from(
          ...
          (script) => this.shouldReloadScript(script)
        );
    ...
    getCurrentScripts() {
      return Array.from(document.querySelectorAll("script")).filter((node) => {
        const script = node;
        return this.shouldReloadScript(script) && !script.hasAttribute("data-soft-reload-ignore");
      });
    }
    shouldReloadScript(script) {
      ...
      const promises = styles.map((style) => {
        return new Promise((resolve, reject) => {
          ...
          if (tag === "link") {
            ...
            link.onload = () => resolve();
            link.onerror = () => reject(new Error(`Failed to load stylesheet: ${source.href}`));
    ...
    async loadScripts(scripts) {
      for (const source of scripts) {
        await new Promise((resolve, reject) => {
          ...
          const parent = source.parentElement?.tagName.toLowerCase() === "head" ? document.head : document.body;
          if (source.src) {
            script.async = false;
            script.defer = false;
            script.src = this.withCacheBust(source.src);
            script.onload = () => resolve();
            script.onerror = () => reject(new Error(`Failed to load script: ${source.src}`));
...
%o`, message, error2, detail);
  ...
  // frontend/src/js/helper/AlertBoxHelper.ts
  var AlertBoxHelper = class {
    constructor(element) {
    ...
    setTopBarProgress(progress) {
      if (progress < 0) progress = 0;
      if (progress > 100) progress = 100;
      if (!this.element) return;
      if (!this.isPresent()) return;
      const topBar = this.element.querySelector(".new-alert-top-bar-content");
      if (!topBar) return;
  ...
  var BaseController = class extends Controller {
    constructor() {
      ...
      __publicField(this, "websocketEndpoints", []);
      __publicField(this, "alertBoxHelper");
    }
    async register() {
    ...
    async handleWebsocket(websocket, event) {
      ...
      if (data.method === "notify_game_update") {
  ...
  var BadgeController = class extends BaseController {
    constructor() {
      super(...arguments);
      __publicField(this, "websocketEndpoints", ["notify_ads"]);
  ...
  function drawParticle(data) {
    const { container, context, particle, delta, colorStyles, backgroundMask, composite, radius, opacity, shadow: shadow2, transform } = data, pos = particle.getPosition(), angle = particle.rotation + (particle.pathRotation ? particle.velocity.angle : defaultAngle), rotateData = {
      sin: Math.sin(angle),
      cos: Math.cos(angle)
  ...
  var Container = class {
    ...
    async init() {
      ...
      const effects2 = this._engine.getSupportedEffects();
      for (const type of effects2) {
  ...
  var BaseEffect = class {
    constructor(content, id, options = {}) {
  ...
  // frontend/src/js/effects/FadeEffect.ts
  var FadeEffect = class extends BaseEffect {
    constructor(content, id = "fade", options = {}) {
    ...
    async handle(element) {
      await this.animate(
        element,
        this.options.reverse ? [{ opacity: 1 }, { opacity: 0 }] : [{ opacity: 0 }, { opacity: 1 }]
      );
    }
  };

  // frontend/src/js/effects/SlideEffect.ts
  var SlideEffect = class extends BaseEffect {
    constructor(content, id = "slide", options = {}) {
      super(content, id, {
  ...
  // frontend/src/js/effects/SlideFadeEffect.ts
  var SlideFadeEffect = class extends BaseEffect {
    constructor(content, id = "slidefade", options = {}) {
      super(content, id, options);
      this.options = {
        ...this.options,
        x: options.x ?? 32,
  ...
  // frontend/src/js/effects/ScaleEffect.ts
  var ScaleEffect = class extends BaseEffect {
    constructor(content, id = "scale", options = {}) {
      super(content, id, options);
      this.options = {
        ...this.options,
  ...
  // frontend/src/js/helper/EffectHelper.ts
  var effects = {};
  ...
  async function triggerEffect(id, element, content, options = {}) {
    const Effect2 = effects[id];
    if (!Effect2) {
      throw new Error(`Effect "${id}" not found`);
    }
    const effect = new Effect2(content, id, options);
  ...
  var AlertController = class extends BaseController {
    constructor() {
      super(...arguments);
      __publicField(this, "websocketEndpoints", ["notify_alert"]);
      __publicField(this, "iconTarget", null);
      __publicField(this, "contentTargets", []);
      __publicField(this, "delayedTargets", []);
      __publicField(this, "contentContainerTarget", null);
      __publicField(this, "videoTarget", null);
      __publicField(this, "logoTarget", null);
      __publicField(this, "imageTarget", null);
      __publicField(this, "iframeTarget", null);
      ...
      for (const element of this.elementTargets) {
        ...
        switch (element.dataset.alertElementType) {
          case "icon":
            ...
            break;
          case "content":
            this.contentTargets.push(element);
            break;
          case "delayed":
            this.delayedTargets.push(element);
            break;
          case "content-container":
            this.contentContainerTarget = element;
            break;
          case "video":
            this.videoTarget = element;
            break;
          case "logo":
            this.logoTarget = element;
            break;
          case "image":
            this.imageTarget = element;
            break;
          case "iframe":
            this.iframeTarget = element;
            break;
        }
      }
    ...
    async runStartEffects() {
      const effects2 = this.parseEffects(this.element.dataset.alertStartEffects);
      if (!effects2.length) {
        this.element.style.opacity = "1";
        return;
      }
      await this.runEffects(effects2);
    }
    async runEndEffects() {
      const effects2 = this.parseEffects(this.element.dataset.alertEndEffects);
      if (!effects2.length) {
        this.element.style.opacity = "0";
        return;
      }
  ...
  __publicField(AlertController, "targets", ["element"]);

  // frontend/src/js/controller/TimerController.ts
  var TimerController = class extends BaseController {
    constructor() {
      super(...arguments);
      __publicField(this, "websocketEndpoints", ["notify_timer"]);
  ...
  __publicField(TimerController, "targets", ["leadingMinute", "lastMinute", "leadingSecond", "lastSecond", "progressBar"]);
  ...
  var EffectController = class extends BaseController {
    constructor() {
      super(...arguments);
      __publicField(this, "websocketEndpoints", ["notify_effect"]);
      __publicField(this, "effectsInitialized", false);
    }
    async postConnect() {
  ...
  var SvgController = class extends BaseController {
    constructor() {
      ...
      __publicField(this, "src", this.element.dataset.src);
    }
    async postConnect() {
      ...
      let hasStyle = true;
      let svgStyleElement = this.element.querySelector("style");
      if (!svgStyleElement) {
        svgStyleElement = document.createElementNS("http://www.w3.org/2000/svg", "style");
  ...
  // frontend/src/js/controller/ToggleController.ts
  var ToggleController = class extends BaseController {
    constructor() {
      super(...arguments);
      __publicField(this, "websocketEndpoints", ["notify_toggle_element"]);
      __publicField(this, "id", this.element.dataset.id);
    }
    async postConnect() {
      this.parseId();
    }
  ...
  // frontend/src/js/controller/ShoutoutController.ts
  var ShoutoutController = class extends BaseController {
    constructor() {
      super(...arguments);
      __publicField(this, "websocketEndpoints", ["notify_shoutout_clip"]);
    }
    async handleMessage(websocket, method, data) {
    ...
    parseData(data, input) {
      return input.replace("${channel}", data.channel).replace("${channelname}", data.name);
    }
  };
  __publicField(ShoutoutController, "targets", ["channelname", "iframe"]);

  // frontend/src/js/controller/InfoController.ts
  var InfoController = class extends BaseController {
    async handleGameUpdate(websocket, data) {
      const gameInfoElements = this.element.querySelectorAll("[data-info]");
  ...
  // frontend/src/js/controller/SourceBackgroundController.ts
  var SourceBackgroundController = class extends BaseController {
    constructor() {
      super(...arguments);
      __publicField(this, "websocketEndpoints", ["notify_source_update"]);
      __publicField(this, "videoExtensions", ["mp4", "webm"]);
      __publicField(this, "imageExtensions", ["gif", "png", "jpg", "webp"]);
    }
    async handleMessage(websocket, method, data) {
  ...
  var MusicController = class extends BaseController {
    constructor() {
      ...
      __publicField(this, "currentTrackId", "");
      __publicField(this, "status", {});
      __publicField(this, "imageElement", this.element.querySelector("#music-thumbnail"));
      __publicField(this, "barElement", this.element.querySelector(".music-progress-bar"));
      __publicField(this, "titleElement", this.element.querySelector(".music-title-text"));
      __publicField(this, "artistElement", this.element.querySelector(".music-artist-text"));
      __publicField(this, "volumeElement", this.element.querySelector(".music-volume"));
      __publicField(this, "showTimeout", -1);
    ...
    async handleMessage(websocket, method, data) {
      ...
      if (this.volumeElement) {
        const volumeBar = this.volumeElement.querySelector(".music-volume-bar");
        const volumeText = this.volumeElement.querySelector(".music-volume-text");
        if (volumeBar) {
  ...
  // frontend/src/js/controller/VisibleController.ts
  var VisibleController = class extends BaseController {
    constructor() {
      super(...arguments);
      __publicField(this, "websocketEndpoints", ["notify_visible_element"]);
      __publicField(this, "id", this.element.dataset.id);
    }
    async handleMessage(websocket, method, data) {
      if (!data) return;
  ...
  // frontend/src/js/controller/MarqueeController.ts
  var MarqueeController = class extends BaseController {
    constructor() {
      super(...arguments);
  ...
  // frontend/src/js/controller/ContenController.ts
  var ContenController = class extends BaseController {
    async preConnect() {
    }
  };

  // frontend/src/js/controller/IFrameController.ts
  var IFrameController = class extends BaseController {
    async preConnect() {
