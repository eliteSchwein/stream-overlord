  ...
  <v-expansion-panel class="macro-obs-task-accordion">
    <v-expansion-panel-title>
      <div class="d-flex align-center min-width-0 w-100">
        <v-icon icon="mdi-keyboard" size="20" class="mr-2" />
        <span class="text-caption mr-2 text-medium-emphasis">#{{ index + 1 }}</span>
        <span class="text-truncate font-weight-medium">{{ title }}</span>
        <v-spacer />
        <v-chip size="x-small" color="primary" variant="tonal">TriggerHotkeyByName</v-chip>
      </div>
    </v-expansion-panel-title>
    ...
    <v-expansion-panel-text>
      <v-row density="comfortable">
        <v-col cols="12" md="6">
          <v-text-field v-model="data.hotkeyName" label="Hotkey name" prepend-inner-icon="mdi-keyboard" variant="outlined" hide-details="auto" />
        </v-col>
      </v-row>
...
export default {
  ...
  created() {
    ...
    this.task.data = this.task.data && typeof this.task.data === 'object' ? this.task.data : {}
      if (this.data.hotkeyName === undefined) this.data.hotkeyName = ''
  },
}
</script>
