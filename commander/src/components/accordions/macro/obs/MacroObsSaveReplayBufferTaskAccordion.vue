  ...
  <v-expansion-panel class="macro-obs-task-accordion">
    <v-expansion-panel-title>
      <div class="d-flex align-center min-width-0 w-100">
        <v-icon icon="mdi-content-save" size="20" class="mr-2" />
        <span class="text-caption mr-2 text-medium-emphasis">#{{ index + 1 }}</span>
        <span class="text-truncate font-weight-medium">{{ title }}</span>
        <v-spacer />
        <v-chip size="x-small" color="primary" variant="tonal">SaveReplayBuffer</v-chip>
      </div>
    </v-expansion-panel-title>
...
export default {
  name: 'MacroObsSaveReplayBufferTaskAccordion',
  ...
  created() {
    ...
    this.task.method = 'SaveReplayBuffer'
